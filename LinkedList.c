        ...
        for (int i =0; i < tam-1; i++)
        {
            for (int j = i+1; j<tam; j++)
            {
                ...
                if ( ( pFunc( auxI, auxJ ) > 0 && order ) ||
                        ( pFunc( auxI, auxJ ) < 0 && !order ) ) // aca va la funcion para comparar que tiene que devolver 1 o 0, o 1 o -1 para swapear
                {
        ...
        if (filterList != NULL)
        {
            ...
            for (int i = 0; i < tam; i++ )
            {
                ...
                if ( pFunc( aux ) )
                {
                    ...
                    {
                        // si falla el add libero filterlist, y null
    ...
    if (this != NULL && pFunc != NULL)
    {
        ...
        if (mapedList != NULL)
        {
            ...
            for (int i = 0; i < tam; i++ )
            {
                ...
                if ( pFunc( aux ) )
                {
                    if (  ll_add(mapedList, aux)  )
                    {
                        // si falla el add libero filterlist, y null
                        ll_deleteLinkedList(mapedList);
                        mapedList = NULL;
                        break;
                    }
                }
